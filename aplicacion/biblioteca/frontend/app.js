...
// Lógica para el buscador de la cabecera (en index.html)
document.getElementById('buscador-avanzado').addEventListener('submit', (e) => {
    e.preventDefault();
    const query = document.getElementById('search-input').value;
    window.location.href = `busqueda.html?q=${encodeURIComponent(query)}`;
});

// Lógica para los botones de filtros (en index.html)
document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
        const filtro = e.target.getAttribute('data-filter');
        // Si el filtro es "all", recarga todo, si no, filtra el array
        if(filtro === 'all') {
            renderBooks(todasLasLecturas);
        } else {
            const filtrados = todasLasLecturas.filter(b => b.type === filtro);
